...
- *Levenshteinova vzdálenost* (resp. její normalizovaná verianta, která kvantifikuje počet editací potřebných k transformaci jednoho řetězce na druhý) pro měření podobnosti většiny textových hodnot (titulek, jména autorů, e-maily),
...
Základní model si s detekcí bounding boxů nedokázal téměř vůbec poradit. Trénováním došlo k výraznému zlepšení, přičemž _fine-tuned_ model dokázal extrahovat ohraničující obdelníky s překryvem (IoU) kolem 70 % u titulků, abstraktů a klíčových slov a 47 % u jmen autorů. Extrakce textových hodnot u trénovaného modelu dosahuje vyšší přesnosti než u trénování bez bounding boxů, což je ale způsobeno hlavně drobnou úpravou evaluačního skriptu, který u modely s bounding boxy spouštěl s méně restriktivním omezením pro maximální počet tokenů v odpovědi.
...
V projektu se podařilo sestavit datovou sady ze serveru arXiv.org o velikosti necelých 50 tisíců článků. Následně k doplnění chybějících hodnot v této datové sadě a také rozšíření metadat a bounding boxy některých z textových hodnot.
...
V souladu s teoretickými předpoklady se nejúspěšnějšími ukázaly modely novější generace. Trénování lze považovat za úspěšné, jelikož všechny natrénované modely dosahovaly výrazně lepších výsledků než jejich základní verze. Nejvýraznější zlepšení jsme zaznamenaly v extrakci institucí autorů, což je náročný úkol vzhledem k faktu, že autoři jsou s jejich institucemi většinou provázání pomocí superskriptů. U této hodnoty dosahovala většina natrénovaných modelů zlepšení o přibližně 40 procentních bodů.

Vůbec nejlépe se trénování osvědčilo v detekci ohraničujících obdelníků, jelikož základní modely s touto úlohou měly velké problémy. Trénování jsme byli schopni dosáhnout přesnosti kolem 80 %.
